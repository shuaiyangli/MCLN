...
class Data(object):
    def __init__(self, path, batch_size):
        ...
        with open(img_feat_file, 'r') as file:
            ...
            for item in self.img_features:
                self.imageFeatMatrix[item] = self.img_features[item]
    ...
    def get_adj_mat(self):
        ...
        try:
            ...
            left = sp.load_npz(self.path + '/adj_mat_left.npz')
            norm_adj_mat_2 = sp.load_npz(self.path + '/adj_mat_2.npz')
            norm_adj_mat_3 = sp.load_npz(self.path + '/adj_mat_3.npz')
            norm_adj_mat_4 = sp.load_npz(self.path + '/adj_mat_4.npz')
            norm_adj_mat_5 = sp.load_npz(self.path + '/adj_mat_5.npz')

            print('already load adj_t matrix', norm_adj_mat_4.shape, time.time() - t1)

        except Exception:
            left, norm_adj_mat_2, norm_adj_mat_3, norm_adj_mat_4, norm_adj_mat_5 = self.create_adj_mat()

            sp.save_npz(self.path + '/adj_mat_left.npz', left)
            sp.save_npz(self.path + '/adj_mat_2.npz', norm_adj_mat_2)
            sp.save_npz(self.path + '/adj_mat_3.npz', norm_adj_mat_3)
            sp.save_npz(self.path + '/adj_mat_4.npz', norm_adj_mat_4)
            sp.save_npz(self.path + '/adj_mat_5.npz', norm_adj_mat_5)

        return left, norm_adj_mat_2, norm_adj_mat_3, norm_adj_mat_4, norm_adj_mat_5
    ...
    def create_adj_mat(self):
            ...
            norm_adj_mat_52 = normalized_adj_symetric(adj_mat + sp.eye(adj_mat.shape[0]), - 0.5, -0.2)
            ...
            norm_adj_mat_52 = norm_adj_mat_52.tocsr()
            ...
            return norm_adj_mat_left.tocsr(), norm_adj_mat_52.tocsr(), norm_adj_mat_53.tocsr(), norm_adj_mat_54.tocsr(), norm_adj_mat_55.tocsr()
    ...
    def sample_u(self):
        ...
        pos_items, neg_items, int_items, pos_users_for_pi, neg_users_for_pi = [], [], [], [], []
        for u in users:
            pos_i = sample_pos_items_for_u(u)
            neg_i = sample_neg_items_for_u(u)
            int_i = sample_int_items_for_u(u)

            pos_items.append(pos_i)
            neg_items.append(neg_i)
            int_items.append(int_i)
    ...
    def test_data(self):
        for u, i in self.test_set.items():
            ...
            while len(negative_items) < 99:
                ...
                if h in self.train_items[u] or h == i:
                    continue
