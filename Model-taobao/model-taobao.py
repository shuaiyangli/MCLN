...
class Model(object):
    ...
    def __init__(self, data_config, img_feat, d1):
        ...
        self.pos_inputs_embeddings = tf.concat([self.pos_i_g_embeddings, self.pos_i_g_embeddings_v], axis=1)
        self.neg_inputs_embeddings = tf.concat([self.neg_i_g_embeddings, self.neg_i_g_embeddings_v], axis=1)
        self.int_inputs_embeddings = tf.concat([self.int_i_g_embeddings, self.int_i_g_embeddings_v], axis=1)
    ...
    def _init_weights(self):
        ...
        all_weights['user_embedding_m'] = tf.Variable(initializer([self.n_users, self.emb_dim]),
                                                      name='user_embedding_m')
    ...
    def counterfactual_learning_layer_1(self, query, key_value, query_int, key_value_int, activation=None, name=None):
        ...
        with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
            ...
            V = tf.layers.dense(key_value, units=2 * self.emb_dim, activation=activation, use_bias=False, name='V')
            K = tf.layers.dense(key_value, units=2 * self.emb_dim, activation=activation, use_bias=False, name='K')
            Q = tf.layers.dense(query, units=2 * self.emb_dim, activation=activation, use_bias=False, name='Q')
            K_int = tf.layers.dense(key_value_int, units=2 * self.emb_dim, activation=activation, use_bias=False, name='K_int')
            Q_int = tf.layers.dense(query_int, units=2 * self.emb_dim, activation=activation, use_bias=False, name='Q_int')

            score = tf.matmul(Q, tf.transpose(K)) / np.sqrt(2 * self.emb_dim)
            score_int = tf.matmul(Q_int, tf.transpose(K_int)) / np.sqrt(2 * self.emb_dim)

            score = score - score_int
            softmax = tf.nn.softmax(score, axis=1)
            attention = tf.matmul(softmax, V)

            counterfactual_learning = tf.layers.dense(attention, units=2 * self.emb_dim, activation=activation, use_bias=False, name='linear')
    ...
    def feed_forward_layer(self, inputs, activation=None, name=None):
        ...
        with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
            ...
            inner_layer = tf.layers.dense(inputs, units=2 * 4 * self.emb_dim, activation=activation)
            dense = tf.layers.dense(inner_layer, units=2 * self.emb_dim, activation=activation)
    ...
    def create_bpr_loss(self):
        ...
        emb_loss = self.decay * (regularizer_mf + regularizer_mf_v + regularizer_mf_m) / self.batch_size

        loss = mf_loss + emb_loss
        self.user_embed = tf.nn.l2_normalize(self.u_g_embeddings_pre, axis=1)
        self.item_embed, self.item_embed_v = tf.nn.l2_normalize(self.pos_i_g_embeddings_pre, axis=1),\
...
if __name__ == '__main__':
    ...
    """
    norm_left, norm_2, norm_3, norm_4, norm_5 = data_generator.get_adj_mat()

    config['norm_adj'] = norm_3

    print('shape of adjacency', norm_3.shape)

    t0 = time.time()

    model = Model(data_config=config, img_feat=data_generator.imageFeatMatrix, d1=4096)
