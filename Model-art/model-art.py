...
class Model(object):
    ...
    def counterfactual_learning_layer_1(self, query, key_value, query_int, key_value_int, activation=None, name=None):
        ...
        with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
            ...
            K_int = tf.layers.dense(key_value_int, units=3 * self.emb_dim, activation=activation, use_bias=False,
                                    name='K_int')
            Q_int = tf.layers.dense(query_int, units=3 * self.emb_dim, activation=activation, use_bias=False,
                                    name='Q_int')
